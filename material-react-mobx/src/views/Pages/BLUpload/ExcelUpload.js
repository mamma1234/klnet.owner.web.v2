...
export default function Excelupload(props) {
    ...
    return new Promise(function(resolve,reject) {
      ...
      axios.post("/loc/getPkMyBlList",
        ...
        if(res.data.length > 0) {
          resolve([<span>{row[0]}<Tooltip title={" 이미 등록 되어있는 B/L 번호"} arrow><IconError color={'error'}></IconError></Tooltip> </span>, row[1], row[2], '이미 저장된', 'B/L번호입니다.' ,false]);
        }else {
          resolve([row[0],row[1],row[2],row[3],row[4],true]);
        }
      })
  ...
  const handleDown = () => {
    ...
    axios.get("/loc/downloadExcel",{responseType:"arraybuffer",headers:{'Authorization':'Bearer '+userStore.token}}).then(
      res => {
