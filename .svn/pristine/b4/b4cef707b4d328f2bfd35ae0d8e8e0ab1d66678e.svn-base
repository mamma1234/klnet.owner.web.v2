...
export default function TableList(props) {
	...
	useEffect(() => {
		axios.post("/sch/getSchedulePortCodeList",{},
						...
						onRowAdd: newData =>
							...
							new Promise(resolve => {
								setTimeout(() => {
									...
									axios.post("/sch/getSchedulePortCodeList",{data:newData},{headers:{'Authorization':'Bearer '+store.token}}).then(res => {
										if ( res.data.length == 0) {
													axios.post("/sch/insertSchPortCode",{newData:newData},{headers:{'Authorization':'Bearer '+store.token}}).then(res => {
									...
									if (oldData) {
										axios.post("/sch/getSchedulePortCodeList",{data:newData},{headers:{'Authorization':'Bearer '+store.token}}).then(res => {
											...
											if ( res.data.length == 0) {
												...
												axios.post("/sch/updateSchPortCode",{ oldData:oldData, newData:newData},{headers:{'Authorization':'Bearer '+store.token}});
												setSelectData(prevState => {
							...
							new Promise(resolve => {
								setTimeout(() => {
									...
									setSelectData(prevState => {
										...
										// 삭제 처리.
										axios.post("/sch/deleteSchPortCode",{ oldData:oldData},{headers:{'Authorization':'Bearer '+store.token}})
										setSearchCount(data.length);
										return { ...prevState, data };
									});
								}, 600);
